...
export async function GET(request: NextRequest) {
  try {
    ...
    // Transform to frontend format and filter out empty questions
    const transformedQuestions = [];

    for (const question of questions) {
      try {
        if (question.questionText?.trim()) {
          transformedQuestions.push(transformQuestion(question));
        }
      } catch (err) {
        // Skip invalid questions with logged error
        console.warn(`Skipping invalid question ${question.id}:`, err);
      }
    }
