...
export function DashboardHeader({ userName }: DashboardHeaderProps) {
  ...
  // Prefer username from our custom auth; fall back to prop; final fallback "Student"
  const displayName = user?.username || userName || 'Student';
    ...
    <header className="hidden lg:flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 lg:gap-0 mb-6 lg:mb-10">
      <div className="w-full lg:w-auto">
        <h1 className="text-2xl lg:text-3xl font-bold mb-2">
          {loading ? 'Loading...' : `Good Afternoon, ${displayName}! 👋`}
        </h1>
      ...
      <div className="flex items-center gap-3 w-full lg:w-auto">
        ...
        <button
          className="glass-panel w-10 h-10 lg:w-12 lg:h-12 rounded-full flex items-center justify-center border border-white/10 flex-shrink-0"
